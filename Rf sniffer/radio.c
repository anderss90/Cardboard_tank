...
#define RX_GND_PIN PORTD3
#define RX_DTA_PIN PIND0
#define RX_VCC_PIN PORTD0

 void rf_init(){
	 // Set up interrupts
	 PCMSK2 |= (1<<PCINT16); //change this when you change pin
	 PCICR |= (1<<PCIE2);
  ...
  void inline starttimer(int v)
  {
	  ...
	  cli();
	  TCNT1 = 0;
	  OCR1A = v; // *2 because this cpu is twice as fast
	  SREG = sreg; // restore interrrupt flag
  ...
  ISR(PCINT2_vect) 
  {
	  ...
	  if (state == 0)
	  {
		  starttimer(kPulseTrainDelay-100*2);
		  return;
	  }
